...
# cv2.destroyWindow 함수에서 윈도우명이 생성한 윈도우명과 달라서 (학번) 생기는 오류 때문에
# cv2.destroyWindow 매개변수명을 일부 수정하였습니다.
...
def HarrisDetector(src, gaus_filter_size = 3, gaus_sigma = 1, alpha = 0.04, threshold_rate = 0.01):
    ...
    cv2.imshow('20211924 - dst_x_Norm', dst_x_Norm)
    cv2.imshow('20211924 - dst_y_Norm', dst_y_Norm)
    cv2.waitKey()
    cv2.destroyWindow('20211924 - dst_x_Norm')
    cv2.destroyWindow('20211924 - dst_y_Norm')
    ...
    cv2.imshow('20211924 - IxIx', IxIx)
    cv2.imshow('20211924 - IyIy', IyIy)
    #cv2.imshow('IxIy', IxIy)
    cv2.imshow('dst_IxIy_Norm', dst_IxIy_Norm)
    cv2.waitKey()
    cv2.destroyWindow('20211924 - IxIx')
    cv2.destroyWindow('20211924 - IyIy')
    ...
    cv2.imshow('20211924 - G_IxIx', G_IxIx)
    cv2.imshow('20211924 - G_IyIy', G_IyIy)
    #cv2.imshow('G_IxIy', G_IxIy)
    cv2.imshow('G_dst_IxIy_Norm', G_dst_IxIy_Norm)
    cv2.waitKey()
    cv2.destroyWindow('20211924 - G_IxIx')
    cv2.destroyWindow('20211924 - G_IyIy')
    ...
    # har 구하기
    """
    har = (G_IxIx * G_IyIy) - (G_IxIy ** 2) - alpha * (((G_IxIx ** 2) + (G_IyIy ** 2)) ** 2)

    #0 ~ 1 사이의 값으로 변경 후 0 ~ 255로 변경 -> 결과가 잘 나왔는지 확인하기위해서
    G_dst_har_Norm = ((har - np.min(har) )/np.max(har - np.min(har)) * 255 + 0.5).astype(np.uint8)
    cv2.imshow('20211924 - har before threshold', G_dst_har_Norm)
    ...
    cv2.imshow('20211924 - har after threshold', G_dst_har_thresh_Norm)
...
def main():
    src = cv2.imread('CV\week4\homework\zebra.png')
    harris_img = src.copy()
    ...
    cv2.imshow('20211924 - interest points', interest_points)
    cv2.imshow('20211924 - harris_img', harris_img)
    cv2.waitKey()

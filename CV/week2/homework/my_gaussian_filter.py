...
def my_get_Gaussian2D_mask(msize, sigma=1):
    ...
    gaussian_filter = gaussian_filter / np.sum(gaussian_filter) # 1을 넘지 않음.
...
def my_get_Gaussian1D_mask(msize, sigma=1):
    ...
    gaus1D = 1 / (np.sqrt(2 * np.pi) * sigma) * np.exp(-((x * x) / (2 * sigma ** 2)))
    gaus1D /= np.sum(gaus1D)
    return gaus1D

if __name__ == '__main__':
    # 본인의 Image path를 맞춰서 넣는 것을 생각할 것
    src = cv2.imread('CV\week2\homework\Lena.png', cv2.IMREAD_GRAYSCALE)

...
def feature_matching_RANSAC(img1, img2, keypoint_num=None, iter_num=1000, threshold_distance=5):
    '''
    :param img1: 변환시킬 이미지
    :param img2: 변환 목표 이미지
    ...
    for i in range(iter_num):
        ...
        random_points = matches_shuffle[:5]

        X = my_ls(random_points, kp1, kp2)
        M = np.array([[X[0][0], X[1][0], X[2][0]],
                  [X[3][0], X[4][0], X[5][0]],
                  [0, 0, 1]])
        i_inliers = 0
        ...
        for match in matches :
            ...
            kp1_trans = (np.linalg.inv(M)).dot(np.array([[kp1_x, kp1_y, 1]]).T)
            kp1_trans_x = kp1_trans[0, 0]
            kp1_trans_y = kp1_trans[1, 0]

            kp1_trans_point = [kp1_trans_x, kp1_trans_y]
            kp2_point = [kp2_x, kp2_y]

            kp1_trans_kp2_distance = L2_distance(np.array(kp1_trans_point), np.array(kp2_point))

            if kp1_trans_kp2_distance < threshold_distance :
                i_inliers += 1
        
        M_list.append(M)
        inliers.append(i_inliers)        

    best_M = M_list[inliers.index(max(inliers))]
...
def main():
    src = cv2.imread('CV/week7/homework/library1.jpg')
    src2 = cv2.imread('CV/week7/homework/library2.jpg')

    result_RANSAC = feature_matching_RANSAC(src, src2)
    result_LS = feature_matching(src, src2)
    cv2.imshow('input', src)
    cv2.imshow('result RANSAC 20211924', result_RANSAC)
    cv2.imshow('result LS 20211924', result_LS)
